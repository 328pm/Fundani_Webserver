...
IMAGE_FILE=Fundani_ws_1.1_img.gz
TGZ_IMAGE_FILE=Fundani_ws_1.1_img.tar.gz
SRC_IMAGE=image_stuff/OpenWRT_ext4_50MB.img.gz
SRC_IMAGE_UNPACKED=image_stuff/Fundani_img
...
$(OPENWRT_CONFIG_FOLDER):
	...
	sed 's:OPENWRT="no":OPENWRT="yes":'  -i $@/Fundani.conf
	sed 's:DO_IFCONFIG="yes":DO_IFCONFIG="no":'  -i $@/Fundani.conf
	sed 's:IPV6_ENABLE="no":IPV6_ENABLE="yes":'  -i $@/ipv6.conf
	sed 's:USE_APN="yes":USE_APN="no":'  -i $@/Fundani.conf
	sed 's:DNSMASQ_INTERFACE="wlan0":DNSMASQ_INTERFACE="br-lan":' -i $@/Fundani.conf
	sed 's:192.168.77:192.168.1:g' -i $@/Fundani.conf
	sed 's:DROOPY_USE_USER="yes":DROOPY_USE_USER="no":' -i  $@/Fundani.conf
	sed 's:DROOPY_CHMOD:#DROOPY_CHMOD:' -i $@/Fundani.conf
	sed 's:LEASE_FILE_LOCATION=$$Fundani_FOLDER/tmp/lease.file:LEASE_FILE_LOCATION=/tmp/lease.file:' -i  $@/Fundani.conf
	sed 's:TIMESAVE_FORMAT="":TIMESAVE_FORMAT="+%C%g%m%d%H%M":' -i $@/Fundani.conf
