...
CONFIG_PATH="/opt/Fundani/conf/hostapd.conf"
Fundani_CONFIG_PATH="/opt/Fundani/conf/Fundani.conf"
...
Fundani_interface=$( grep -e '^INTERFACE' "${Fundani_CONFIG_PATH}" | \
                sed -e 's|INTERFACE=||' -e 's|"||g' )
dnsmasq_interface=$( grep -e '^DNSMASQ_INTERFACE' "${Fundani_CONFIG_PATH}" | \
                sed -e 's|DNSMASQ_INTERFACE=||' -e 's|"||g' )

sed -i -e "s|interface=$hostap_interface|interface=$WIFI_DEVICE|" "${CONFIG_PATH}"

#Only change Fundani interface if it is a wifi interface
if echo "$Fundani_interface" | grep -q "wlan" ; then
    sed -i -e "s|INTERFACE=\"$Fundani_interface\"|INTERFACE=\"$WIFI_DEVICE\"|" \
         "${Fundani_CONFIG_PATH}"
fi
...
if [ -f /boot/wpa_supplicant.conf ]; then
  /opt/Fundani/rpi/bin/run_client.sh
  exit 1
  # Exit =! 0 will result in not starting Fundani service
fi
