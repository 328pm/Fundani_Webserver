...
if [ -z $1 ] || [ -z $2 ] ; then
  ...
  echo "Usage $0 <path to Fundani.conf> <step>"
...
if [ "$2" = "install" ] ; then
  ...
  echo "#--- Crontab for Fundani-Timesave" >> $Fundani_FOLDER/tmp/crontab
  echo " */5 * * * * $Fundani_FOLDER/bin/timesave.sh $Fundani_FOLDER/conf/Fundani.conf save" >> $Fundani_FOLDER/tmp/crontab
  ...
  echo "  on OpenWrt run: /etc/init.d/Fundani enable"
