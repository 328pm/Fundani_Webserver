...
#    licenced with GPL-3
#
# Generate severall configuration files out of Fundani.conf
...
generate_lighttpd_env() {
	...
	LIGHTTPD_ENV="setenv.add-environment = (
	   \"PYTHONPATH\"             => \"$PYTHONPATH:$Fundani/python_lib\",
...
if [ -z  $1 ] ; then
  echo "Usage is
      generate_config_files.sh /opt/Fundani/conf/Fundani.conf
   "
   exit 255
fi
...
. $NODE_CONFIG
. $Fundani_FOLDER/lib/node_name_generation.sh

IPV6="#"

set_pathnames  $Fundani_FOLDER
...
generate_lighttpd_env $GLOBAL_CHAT "$GLOBAL_DEST" $Fundani_PYTHONPATH $GEN_CHATFILE $Fundani_FOLDER  $CHATFILE $SHOUTBOX_CLIENT_TIMESTAMP $UPLOADFOLDER $GEN_DISKFILE
...
### Fundani Scripts generates its own config in  /opt/Fundani/conf/avahi
###   but, the daemon works per default only on /etc/avahi
### If you want to enable avahi, then you have to link /etc/avahi to /opt/Fundani/conf/avahi
### On OpenWRT this should happen, if avahi is available before installing the Fundani
###  automtically.
